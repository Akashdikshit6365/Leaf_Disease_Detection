...
export default function HeatmapViewer({ imageUrl, heatmapUrl }) {
  ...
  if (!imageUrl && !heatmapUrl) {
    return (
      <div className="enterprise-shell p-5 md:p-8 reveal-fade">
        ...
        <div className="relative space-y-6">
          ...
          <div className="rounded-[16px] border border-dashed border-white/15 bg-white/[0.02] p-10 text-center">
  ...
  return (
    <div className="enterprise-shell reveal-fade p-5 md:p-8">
      <div className="ambient-frame" />
      ...
      <div className="relative space-y-8">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="flex-1">
            ...
            <p className="mt-3 max-w-xl text-base leading-8 text-white/62">
              Review the visual areas that most strongly support the diagnosis. Use this as evidence, not a replacement for field judgment.
            </p>
          </div>

          <div className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.03] p-1.5 backdrop-blur">
            {['overlay', 'split'].map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`rounded-lg px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.18em] transition-all duration-300 capitalize ${
                  mode === m 
        ...
        <div className="relative overflow-hidden rounded-[18px] border border-white/10 bg-black/80">
          ...
          {mode === 'overlay' ? (
            <div className="relative aspect-square">
              ...
              <div className="absolute inset-4 rounded-[14px] border border-white/10 pointer-events-none" />
        ...
        {mode === 'overlay' && heatmapUrl && (
          <div className="rounded-[16px] border border-white/8 bg-white/[0.03] p-5 space-y-5">
